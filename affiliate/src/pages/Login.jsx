...
const Login = () => {
  ...
  return (
    <div className="min-h-screen font-poppins flex items-center justify-center p-4 bg-gray-50">
      <Toaster position="top-center" />
      
      <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md relative overflow-hidden border border-gray-200">
        {/* Custom Loading Overlay */}
        {loading && (
          <div className="absolute inset-0 z-10 bg-white flex items-center justify-center rounded-xl">
            <div className="flex flex-col items-center">
              <div className="loader"></div>
              <p className="text-white mt-4 text-sm">Signing in...</p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-600 text-sm mt-2 text-center">Sign in to your master affiliate account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 md:px-4 md:py-3 bg-white border outline-theme_color border-gray-200 rounded-[5px] ${
                errors.email ? 'border-red-300' : 'border-blue-400'
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`w-full px-3 py-2 md:px-4 md:py-3 bg-white outline-theme_color border  border-gray-200 rounded-[5px] ${
                errors.password ? 'border-red-300' : 'border-blue-400'
              }  transition duration-200  text-sm md:text-base`}
              disabled={loading}
            />
            {errors.password && (
          ...
          <button
            type="submit"
            className="w-full text-white font-medium py-3 rounded-[5px] cursor-pointer transition-all duration-200  bg-theme_color text-sm md:text-base"
            disabled={loading}
          >
            {loading ? (
              <span className="flex items-center justify-center text-sm">
                <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-theme_color" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
