...
const Masteraffiliatedetails = () => {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`${base_url}/api/affiliate/master-affiliates/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
        }
                          ...
                          <tr key={user._id || index} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                            ...
                            <td className="py-4 px-6">
                              <button
                                ...
                                className="flex items-center px-3 py-1 bg-orange-500 text-white text-sm rounded hover:bg-orange-600 transition-colors"
                              >
